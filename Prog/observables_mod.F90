...
!>  For unequal time correlation functions, tau runs from 1,Ltrot+1
      ...
      Subroutine Obser_Latt_local_make(Obs, Nt, Filename, Latt, Latt_unit, Channel, dtau)
!--------------------------------------------------------------------
!> @author
!> ALF Collaboration
!>
...
!> observable.
!>
!> @param [INOUT] Obs, Type(Obser_Latt_local)
!> \verbatim
...
!-------------------------------------------------------------------
         ...
         If (Nt > 1) then
            write(error_unit,*) "Nt > 1 not yet implemented for local observables"
            CALL Terminate_on_error(ERROR_HAMILTONIAN,__FILE__,__LINE__)
         endif
         Allocate (Obs%Obs_Latt(Latt%N, Nt, Latt_unit%Norb))
         Obs%File_Latt = Filename
         Obs%Latt => Latt
         Obs%Latt_unit => Latt_unit
         Obs%Channel = Channel
         Obs%dtau = dtau
      end subroutine Obser_Latt_local_make
!--------------------------------------------------------------------
         
      Subroutine Obser_Latt_local_Init(Obs)
         Implicit none
         class(Obser_Latt_local), intent(INOUT) :: Obs
         Obs%Obs_Latt  = cmplx(0.d0,0.d0,kind(0.d0))
         Obs%N         = 0
         Obs%Ave_Sign  = 0.d0
      end subroutine Obser_Latt_Local_Init
...
!--------------------------------------------------------------------
         Subroutine  Print_bin_Latt_Local(Obs, Group_Comm)
            ...
            Ntau  = Size(Obs%Obs_Latt,2)
            if ( .not. (Obs%Latt%N == Ns ) ) then
               Write(error_unit,*) 'Error in Print_bin_Latt_Local'
               CALL Terminate_on_error(ERROR_GENERIC,__FILE__,__LINE__)
            endif
            If (Ntau == 1) then
               File_suff = "_local"
            ...
            If (Irank_g == 0 ) then
               ...
               do I = 1, Obs%Latt%N
                  x_r = dble(Obs%Latt%list(i,1))*Obs%Latt%a1_p + dble(Obs%Latt%list(i,2))*Obs%Latt%a2_p
                  Write(10, '(E25.17E3, 1x, E25.17E3)') X_r(1), X_r(2)
                  Do nt = 1, Ntau
                     do no = 1, Obs%Latt_unit%Norb
                        Write(10, '("(", E25.17E3, ",", E25.17E3, ")")') Obs%Obs_Latt(I,nt,no)
                     enddo
                  enddo
               enddo
               close(10)
               ...
               if ( .not. link_exists ) then
                 ...
                 !Create Dataset for data
                 allocate( dims(5) )
                 dims = [2, Obs%Latt%N, Ntau, Obs%Latt_unit%Norb, 0]
                 CALL init_dset(file_id, obs_dsetname, dims, .true.)
                 deallocate( dims )
               ...
               !Write data
               dat_ptr = C_LOC(Obs%Obs_Latt(1,1,1))
               CALL append_dat(file_id, obs_dsetname, dat_ptr)
         ...
         End Subroutine Print_bin_Latt_Local
!--------------------------------------------------------------------
