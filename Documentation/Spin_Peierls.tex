...
\begin{equation}
    \left[   \hat{S}^{\alpha}_i , \hat{S}^{\beta}_j \right]   =    \delta_{i,j}  i  \sum_{\gamma=1}^{3} \epsilon^{\alpha  \beta \gamma}  \hat{S}_{\gamma},  \, \, \, \,  
     \left(   \ve{\hat{S}}_i  \right)^2  = \frac{3}{4}, \, \, \, \,  \text{and} \,  \, \, \,  \left[ \hat{P}_b,  \hat{Q}_{b'}  \right]   =   \frac{ \delta_{b,b'} } {i}.      
\end{equation}
...
We  have  tested  the  code  for  an  $L=16 $ chain    at  $ \beta J  = 8 $,  $\omega_0 = 0.25 $ and    various  values  of     the  electron-phonon  interaction.  As  can be seen  in   Fig.~\ref{Spin_Peierls.fig}   the  results  compare   remarkably  well  to  independent  calculations  based on the  so  called   SSE  algorithm  with  retarded interactions  \cite{Weber21}.    The  Hamiltonian  \texttt{Hamiltonian\_Spin\_Peierls\_smod.F90}    includes   spin-spin   correlations  as  well  as  phonon  correlations,  both  at  equal and   imaginary  time  displaced.  Scalar  observables  $\texttt{Phi\_scal}$  contain the  $\left<  \hat{Q}_b \right>$   for  bond  in the  x- and  y-directions.
...
\includegraphics[width=0.49\textwidth]{Spin_Peierls/SP_Lambda0.3.pdf}
        ...
        reference  data   was  produced    with the  SSE    algorithm   with  retarded  interaction  introduced  in  Ref.~\cite{Weber17}   and  adapted to   spin-Peierls problems  in \cite{Weber21}.   As  apparent  the  agreement is  excellent,  both for  the  Heisenberg  chain at   $\lambda = 0.0 $    and  at finite  electron phonon coupling,    $\lambda = 0.3 $      }  
