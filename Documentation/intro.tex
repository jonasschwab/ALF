...
The formulations of  the auxiliary-field approach  can  be  generalized  to   time  dependent   Hamiltonians.   This  
allows    for  great  flexibility  and in particular  to  use  the  projective  algorithm  as  a  variational approach   \cite{Sorella21}. 
...
\end{itemize}
The bosonic  part of the general Hamiltonian (\ref{eqn:general_ham}) is 
$ \hat{\mathcal{H}}_{0,I} $ and  has the following properties:
\begin{itemize}
\item $\hat{Z}_k$  couples to a general one-body term  and     $\hat{Q}_k$   to  a    two-body   operator. 
We will work in a basis where these operators are  diagonal:  $\hat{Z}_k | \phi \rangle  =  \phi_k | \phi \rangle $,  
$\hat{Q}_k | \varphi \rangle  =  \varphi_k | \varphi \rangle $.   
$\phi_k$ is a real  number   or an  Ising variable. 
Hence   $\hat{Z}_k$ can correspond  to the Pauli matrix $\hat{\sigma}_{z}$ or to the position operator.    
The  same applies  for  $\varphi_k$ 
\item  The dynamics of the bosonic fields   is given by $\hat{\mathcal{H}}_{0,I}$. This term is not specified here; 
it has to be specified by the user and becomes relevant when the Monte Carlo update probability is computed in the code
\end{itemize}
Note that the matrices  $\bm{T}^{(ks)}$,  $\bm{V}^{(ks)}$, $\bm{\tilde{V}}^{(ks)}$ and  $\bm{I}^{(ks)}$ explicitly depend on the flavor index $s$ but not on the color index $\sigma$. 
...
the matrices $\bm{T}^{(ks)}$,  $\bm{V}^{(ks)}$, $\bm{\tilde{V}}^{(ks)}$ and  $\bm{I}^{(ks)}$  to be  Hermitian.   In the  description of the code will drop  the $\cal{H}_Q$ term since it merely obscures the notation without adding any new aspects. This feature of the code will be used in the spin-Peierls Hamiltonian example \ref{Spin_Peierls.Sec}.
...
library  to facilitate  implementation of new  Hamiltonians. 

The package comes with a set of Hamiltonians, described in Sec.~\ref{sec:model_classes}, which includes: (i) SU(N) Hubbard models allowing pinning fields \cite{Assaad13}, (ii) SU(N) t-V models, (iii) SU(N) Kondo lattice models, (iv)    Models with long ranged coulomb interactions,  (v) Generic $\Ztwo$ lattice gauge theories coupled to $\Ztwo$ matter and fermions and (vi)  models with spin-Peierls interactions.  These model classes are built on the predefined structures. 

In Sec.~\ref{sec:maxent}  we  describe how to use our implementation of the stochastic analytical  continuation \cite{Sandvik98,Beach04a} as well as the maximum entropy method \cite{Jarrell96,Linden95}. These methods allow one to obtain spectral functions from imaginary-time correlation functions measured in the QMC simulations.  One can switch between both methods by merely changing a parameter in the input file. 
