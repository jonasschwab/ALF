...
As  mentioned in  Sec.~\ref{sec:obs}  and  summarized in  Table~\ref{table:Obser_latt}   the  lattice  and  unit-cell   objects  that  are  passed  in  the  following  routines,  are  tied  to  the  specific  observable   and  not    to  the  Hamiltonian.     For  example,    the  Hamiltonian   can be  defined on a    bilayer-honeycomb lattice,  with  associated  \textit{hamiltonian} lattice  \texttt{Latt\_ham},  unit-cell  \texttt{Latt\_unit\_ham},   and   lists \texttt{list\_ham}, \texttt{invlist\_ham},  whereas the  observable can be defined  on  a one  dimensional  \textit{observable} lattice  \texttt{Latt} with a   two orbitals per  unit  cell  \texttt{Unit\_cell}.     Recall  that  the  total number of  orbitals  reads
\begin{lstlisting}[style=fortran]
 Ndim  =  Latt_ham%N * Latt_unit_ham%Norb 
\end{lstlisting}
and  that  the  Green  functions  have  dimension  $ \texttt{Ndim}  \times   \texttt{Ndim}   \times   \texttt{N\_fl}  $.      The  array    \texttt{list(Ndim, 2)} is   defined  as  follows:  
\begin{equation}
	  \texttt{list(n,1)} =  
\left\{ 
\begin{array}{ll}
0  &  \text{if  n   is  not  an  orbital of  the  \textit{observable} lattice }   \\
i_{unit}   &  \text{otherwise. }  i_{unit} \text{ corresponds  to  the   unit  cell of the \textit{observable} lattice } 
\end{array}
\right. 	  
\end{equation}
\begin{equation}
...
\begin{array}{ll}
0    &  \text{if  n   is  not  an  orbital of  the \textit{observable}  lattice }   \\
i_{orb}  &  \text{otherwise. }   i_{orb}  \text{ corresponds  to  the  orbital  of  the \textit{observable}  lattice } 
\end{array}
\right. 	  
\end{equation}
...
$\ve{j}$   corresponds to the unit-cell index and $n_{\ve{j}}$    labels the orbital. \footnote{Recall  that   $  \langle \langle \bullet \rangle \rangle_C $   corresponds  to   the    expectation value  of  an  observable  for a  given  configuration of  fields  $C$  and  not  to the connected   part  of  the  observable, see Eq.~(\ref{eqn:obs}).}
...
Note  that  for each observable the  background   is  stored  in the $\texttt{Obs\_latt0}$ field, see Table~\ref{table:Obser_latt}.
...
Which returns:
\begin{align}
\texttt{Obs}(\ve{i}-\ve{j}, n_{\ve{i}}, n_{\ve{j}}) = \langle  \langle\hat{ N}_{\ve{i},n_{\ve{i}}}  \hat{N}_{\ve{j},n_{\ve{j}}}\rangle \rangle \rangle_C,
\end{align}
where
\begin{align}
\label{Density_op.eq}
...
\end{align}
The  background,   $\langle  \langle\hat{ N}_{\ve{i},n_{\ve{i}}} \rangle \rangle_C$   is  stored  in  $\texttt{Obs0}$. 
...
Which returns:
\begin{align}
\texttt{Obs}(\ve{i} - \ve{j}, \tau, n_{\ve{i}},  n_{\ve{j}} )  = \sum_{\sigma=1}^{N_\text{col}} \sum_{s=1}^{N_\text{fl}} \langle \langle  \hat{c}^{\phantom\dagger}_{\ve{i},n_{\ve{i}},  \sigma,s}(\tau) \hat{c}^{\dagger}_{\ve{j}, n_{\ve{j}},\sigma,s} \rangle \rangle_{C}
\end{align}
...
\end{lstlisting}
\begin{align}
\texttt{Obs}(\ve{i} - \ve{j}, \tau, n_{\ve{i}},  n_{\ve{j}} )= \frac{2N}{N^2-1}\sum_{a=1}^{N^2 - 1}  \langle \langle \ve{\hat{c}}^{\dagger}_{\ve{i}, n_{\ve{i}}}(\tau) T^a \ve{\hat{c}}^{\phantom\dagger}_{\ve{i}, n_{\ve{i}}}(\tau)  \;
...
%\\  
%- 4 \langle \langle \ve{\hat{c}}^{\dagger}_{\ve{i}, n_{\ve{i}}} S^z \ve{\hat{c}}^{\phantom\dagger}_{\ve{i}, n_{\ve{i}} } \rangle \rangle_{C}  \langle \langle \  \ve{\hat{c}}^{\dagger}_{\ve{j}, n_{\ve{j}}} S^z  \ve{\hat{c}}^{\phantom\dagger}_{\ve{j}, n_{\ve{j}}}\rangle \rangle_{C}   
\end{multlined} \nonumber \\  
...
Which returns:
\begin{align}
\texttt{Obs}(\ve{i} - \ve{j}, \tau, n_{\ve{i}},  n_{\ve{j}} ) = \langle  \langle \hat{ N}_{\ve{i},n_{\ve{i}}} (\tau)  \hat{N}_{\ve{j},n_{\ve{j}}}\rangle\rangle_C. 
 \end{align}
...
%-----------------------------------------------------------------------------------
\subsubsection{Bond-Bond  correlations}
%-----------------------------------------------------------------------------------
The  function 
\begin{lstlisting}[style=fortran]
 Complex (Kind=Kind(0.d0)) function Predefined_Obs_dimer_kin_eq(I,M, J, N,  GR, GRC, 
                                                                N_SUN, N_FL) 
\end{lstlisting}
returns:
\begin{equation}  
  \left< \left< \hat{K}_{i,m}  \hat{K}_{j,n}    \right>  \right>_C    \, \, \,   \text{with} \, \, \,  \hat{K}_{i,m}   = 
  \sum_{s=1}^{N_{Fl}} \sum_{\sigma=1}^{N_{col}}  \hat{c}^{\dagger}_{i,s,\sigma}  \hat{c}^{\phantom\dagger}_{m,s, \sigma}   + \text{H.c.} 
\end{equation}
...
\begin{equation}
	 \left< \left< \hat{K}_{i,m}    \right>  \right>_C.
\end{equation}

%-----------------------------------------------------------------------------------
\subsubsection{Expectation value of perfect  squares}
%-----------------------------------------------------------------------------------
The  function
\begin{lstlisting}[style=fortran]
 Complex (Kind=Kind(0.d0)) function Predefined_Obs_V_Int(OP_Vint, GR, GRC, N_SUN )
\end{lstlisting}
returns,   for  a  \texttt{OP\_Vint\%type=2}      vertex,   the   expectation value: 
\begin{equation}
	\left< \left<     \left[  \sum_{s=1}^{N_{fl}} \sum_{\sigma=1}^{N_{col}}  \left(   \left(  \sum_{x,y} \hat{c}^{\dagger}_{x,s,\sigma}  V^{s}_{x,y} 
	 \hat{c}^{\phantom\dagger}_{y,s,\sigma} \right)     +  \alpha_s \right)  \right] ^2    \right>  \right>_C
\end{equation}
The   function is   particularly  useful  in computing  the    total  energy  of  the model  at hand. 
