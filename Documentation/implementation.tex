...
\item[\texttt{t=4}]   This  type   allows   for  multiple  HS   fields  per   operator.   It is  however  limited  to  the  
form 
\begin{equation}
	  \gamma(l)  e^{ \sqrt{ 1 + \phi}  \,   \eta(l) \,  \bm{X} }, 
\end{equation}
where  $\phi$ is  a  continuous   real  variable     generically,  but not  necessarily,  bounded  by  $-1 $.
\end{itemize}
