...
\begin{table}[h!]
    ...
    \hl{\texttt{Op\_X\%g}}       & \texttt{cmplx}   &  Coupling strength $g$ \\  
     \hl{\texttt{Op\_X\%g\_t}}       & \texttt{cmplx}   &  Time dependent coupling strength $g(\tau)$ \\  
...
The partition function (see Sec.~\ref{sec:method}) consists of terms which, in general, can be written as $\gamma e^{g \phi \bm{X} }$, where $\bm{X}$ denotes an arbitrary operator, $g$ is a constant, and $\gamma$ and $\phi$ are   functions  of  a  complex  field  $f$. 
The ALF includes three different types of fields:
\begin{itemize}
\item[\texttt{t=1}]   This type is for an  Ising field, therefore $\gamma= 1$ and $ \phi = \Re f =  \pm 1$,
\item[\texttt{t=2}]   This type is for the generic HS  transformation of Eq.~\eqref{HS_squares} where  $\gamma \equiv \gamma(\Re f) $  and $ \phi = \eta(\Re f)$  with $\Re f  = \pm 1, \pm 2$ [see Eq.~\eqref{eta_gamma_fields}],
\item[\texttt{t=3}]   This type is for continuous fields, i.e., $\gamma= 1$  and $ \phi   =  \Re f  \in \mathbb R $.
\item[\texttt{t=4}]    For  this  type  both  real and  imaginary  part  of   the  field   $\phi$ is  used.    It is  limited  to  the  
form 
	$\gamma \equiv \gamma(\Re f)$  and  $  \phi =   \sqrt{ 1 + \Im f}  \,   \eta(\Re f ) $, 
with $ \Re{f} =  \pm 1, \pm 2$    and   $ \Im{f}  \in \mathbb{R} $.   This   type  is  useful     for  simulating    terms  of    the  form: $  g (1 +  \hat{Z})  \hat{O}^2 $  where 
in a  basis   where  $\hat{Z} | f  \rangle  =     \Im  f   |  f  \rangle $  with  $ \Im f  \in \mathbb{R} $. 
\end{itemize}
...
\begin{table}[h!]
	...
	\begin{tabular}{@{} p{0.25\columnwidth} p{0.05\columnwidth} p{\dimexpr0.67\columnwidth-2\tabcolsep\relax} @{}}\toprule
		...
		\texttt{Field\%f(1:n\_op, 1:Ltrot)} & \texttt{cmplx}  & Defines the auxiliary fields. The first index runs through the operator sequence and the second through the time slices. For \texttt{t=1}, $f = \pm 1$; for \texttt{t=2}, $f = \pm 1, \pm 2$;  for \texttt{t=3}, $f \in \mathbb{R}$; and  for \texttt{t=4},  $f \in \mathbb{C}$,    $ \text{Real}(f) = \pm 1, \pm 2 $, $\text{Im}(f) \in \mathbb{R} $  \\
		\texttt{Field\%flip\_protocol(1:n\_op)}         &  \texttt{int}  & Gives  the possibility  to  choose  the  flipping  protocol of  the field for type \texttt{t=4}  fields.    
		                          1:   real  or   imaginary  parts  of  the field  are   updated  with   0.5  probability (default)
		                          2:   both real  and imaginary  parts  are  updated 
		                          3:   only  real  part  4: only  imaginary  part. \\
		\texttt{amplitude}                 & \texttt{dble}  & Random flip width for fields of types \texttt{t=3, 4},   defaults to \texttt{1} \vspace{8pt} \\ 
		...
		three other values of $l$. For  \texttt{t=3},  \texttt{f = f + amplitude*(ranf() -1/2)}.  For  $ \texttt{t=4} $,  the real part  is   flipped as for  
		$ \texttt{t=2} $,  and  the imaginary  part  as  for  $ \texttt{t=3} $.  The  specifics  of  the  flip is encoded in the  \texttt{flip\_protocol}  variable   described  above. \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%phi(n,nt)}}          & Returns $\phi$ for the \texttt{n}-th operator at the time slice  \texttt{nt}.  
		 For \texttt{t=1, 3} $ \phi=f $; 
		 for  \texttt{t=2} $\phi =  \eta(\Re{f})$ ;
		 and  for   \texttt{t=4} $\phi =  \sqrt{1  +  \Im f}  \eta(\Re{f})$  \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%gamma(n,nt)}}        &
		 Returns $\gamma$ for the  \texttt{n}-th operator at the time slice  \texttt{nt}  
		 For \texttt{t=1,3}  $\gamma= 1 $; 
		 and for  \texttt{t=2,4} $\gamma=  \gamma(\Re{f})$  \\
...
%--------------------------------------------------------------------------------------------
\subsection{The observable types \texttt{Obser\_Vec}, \texttt{Obser\_Local} and \texttt{Obser\_Latt}}\label{sec:obs}
%--------------------------------------------------------------------------------------------
...
%------------------------------------------------------------
Local observables are stored in the data type \texttt{Obser\_Local} described in Table \ref{table:Obser_local}. This data structure is useful for dealing with local observables of the form  $\hat{O}_{i,\alpha} $, where $i$ is a lattice site and $\alpha$ an orbital index. No translation symmetry is  implied such that it is possible to use it in conjunction with pinning fields \cite{Assaad13}.   Note that this routine is only implemented for equal-time correlation functions.

Consider a variable  \texttt{Obs} of type  \texttt{Obser\_local}. At the beginning of each bin a call to  \texttt{Obser\_Latt\_Init} in the module \texttt{observables\_mod.F90}  will  initialize  the elements of \texttt{Obs} to zero.    Each time the main program calls the   \texttt{Obser} routines  at time slice $\tau$ one accumulates $ \langle \langle \hat{O}_{\vec{i},\alpha} (\tau)  \rangle \rangle_{C} \allowbreak \frac{e^{-S(C)}} {\Re \left[e^{-S(C)} \right]}  \sgn(C) $   in  \texttt{Obs\%Obs\_latt($\vec{i},1,\alpha$)}  
At the end of each bin, a call to \texttt{Print\_bin\_Local} in the module  \texttt{observables\_mod.F90}   will append the result of the bin in the specified  file \texttt{Obs\%File\_Latt}.  The routine does not carry out a Fourier transformation and prints the data in real space. 
We have adopted the following naming conventions:
For equal-time observables, defined by having the second dimension of the array  \texttt{Obs\%Obs\_local($\vec{i},\tau,\alpha$)}   set to unity, 
the routine \texttt{Print\_bin\_Latt}  attaches the suffix \emph{\_eq} to \texttt{Obs\%File\_Latt}.  Time displaced local observables are not yet implemented. 
Furthermore, \texttt{Print\_bin\_Latt} will create a corresponding info file with suffix \emph{\_eq\_info}, if not already present. The info file contains the channel, number of imaginary time steps, length of one imaginary time step, unit cell and the vectors defining the Bravais lattice.
...
\begin{table}[h!]
	...
	\begin{tabular}{@{} p{0.22\columnwidth}  l  p{0.30\columnwidth} p{0.26\columnwidth} @{}}\toprule
		...
		\texttt{Obs\%Obs\_latt($\vec{i}, \allowbreak \tau,\alpha$)}        & \texttt{cmplx}      &    Cumulated correlation function & 
      $ \begin{multlined}[t] \langle \langle \hat{O}_{\vec{i},\alpha} (\tau) \rangle \rangle_{C} \times\\[-0.5ex] \tfrac{e^{-S(C)}} {\Re \left[e^{-S(C)} \right]}  \sgn(C) \end{multlined}$  
      \\
		...
		\\\bottomrule
	\end{tabular}
	\caption{Components of a variable of type \texttt{Obser\_local} named \texttt{Obs}. 
	Be aware: The types marked with asterisks, $*$, are actually pointers; when the subroutine \texttt{Obser\_Latt\_make} creates an observable \texttt{Obs}, the variables \texttt{Latt} and \texttt{Latt\_unit} do not get copied but a pointer to them is stored, implying that their modification
after the creation of \texttt{Obs} still affects the observable.    Each  observable  can have its  own lattice  and  unit-cell  that  can  differ  from  the lattice  and  unit  cell  associated to the Hamiltonian. For  example,    when  simulation a  two-dimensional  Hubbard model,  one  can  define an observable on a one-dimensional sub-lattice. We refer the reader to Sec.~\ref{sec:predefined_observales} for further discussion on this option.}
	\label{table:Obser_local}
\end{center}
\end{table}
%%------------------------------------------------------------
...
%   lll}
   \toprule
    ...
    \texttt{Calc\_Fl}         & \path{logical}          & Explicitly calculation of flavors (optional) \\
     \bottomrule
   \toprule
    Private Variable          &  Type                   & Description \\
    \midrule
    \texttt{Obs\_scal}        & \path{Obser_Vec}        & Storage for measured scalar observables \\
    \texttt{Obs\_eq}          & \path{Obser_Latt}       & Storage for measured equal time correlations \\
    \texttt{Obs\_tau}         & \path{Obser_Latt}       & Storage for measured time displaced correlations \\
     \bottomrule
   \end{tabular}
...
\begin{center}
 \begin{tabular}{ p{0.25\columnwidth} p{0.51\columnwidth} p{0.15\columnwidth} } 
   ...
   \texttt{Get\_Delta\_S0\_global} &
      Computes $\Delta S_0=S_0(C)-S_0(C')$ for a global move  &
         \ref{sec:global_slice} \\ 
   \texttt{Init\_obs}  &
   ...
   \texttt{Pr\_obs}   &
      ...
      Usually this does not have to be modified &   \\    
      \texttt{weight\_reconstruction}   &
      Reconstructs of the weight for `inactive' flavors &  \ref{sec:flavor_sym}  \\    
      \texttt{GR\_reconstruction}   &
      Reconstructs the Green function \texttt{GR} for the `inactive' flavors &  \ref{sec:flavor_sym}  \\    
      \texttt{GRT\_reconstruction}   &
      Reconstructs the time-displaced Green functions \texttt{G0T} and \texttt{GT0} & \ref{sec:flavor_sym}
 \\\bottomrule   
 \end{tabular}
...
Here, a pair of flavors $(n_\mathrm{fl},\bar{n}_\mathrm{fl})$ are related by a unitary or anti-unitary symmetry, including particle-hole transformations, such that
\begin{equation}
	\mathcal{U}^{-1} \texttt{Op\_V(i,n$_\mathrm{fl}$)} \mathcal{U} = \texttt{Op\_V(i,$\bar{n}_\mathrm{fl}$)}; \quad 
	\mathcal{U}^{-1} \texttt{Op\_T(i,n$_\mathrm{fl}$)} \mathcal{U} = \texttt{Op\_T(i,$\bar{n}_\mathrm{fl}$)}
\end{equation}
for any given $i$. Note that \texttt{Op\_V(i,n$_\mathrm{fl}$)} includes a constant shift $\alpha$ to absorb contributions from the commutator in case of particle-hole symmetries.
...
This acceleration is activated by allocating \texttt{Calc\_Fl} in \texttt{Ham\_set} and setting the `active' flavors $n_\mathrm{fl}$ to \texttt{.True.} and the symmetry-related flavors $\bar{n}_\mathrm{fl}$ to \texttt{.False.}.


This symmetry allows one to reconstruct one flavor, say $\bar{n}_\mathrm{fl}$, from the other, $n_\mathrm{fl}$. For unitary symmetries, the weight is given by $Z(\bar{n}_\mathrm{fl})=Z(n_\mathrm{fl})$, while anti-unitary symmetries lead to $Z(\bar{n}_\mathrm{fl})=Z(n_\mathrm{fl})^*$. This relation has to be provided by the user in  \texttt{weight\_reconstruction}. Up to entry, all weights $Z(n_\mathrm{fl})$ have be explicitly calculated by ALF and the user has to fill all `inactive' flavors $\bar{n}_\mathrm{fl}$ of the array $Z$.
Similarly, the subroutines \texttt{GR\_reconstruction} and \texttt{GRT\_reconstruction} have to be overridden to provide the symmetry reconstruction of the `inactive' flavors $\bar{n}_\mathrm{fl}$ of the equal-time and time-displaced Green functions out of $n_\mathrm{fl}$, respectively.

Finally  we  note  that   if  the projective  algorithm is  used  then   the  trial  wave  function  also has  to satisfy  the   aforementioned  symmetry.
In particular,
assume  that  the   trial wave function    corresponds  to the  ground state of a  single  particle  Hamiltonian,   $\hat{H}_T(n_\mathrm{fl}) $,  then  we
will require  that
\begin{equation}
\mathcal{U}^{-1}   \hat{H}_T(n_\mathrm{fl})  \mathcal{U} = \hat{H}_T(\bar{n}_\mathrm{fl})
\end{equation}

\subsubsection{Time-dependent Hamiltonians}\label{sec:time}
\input{time_dependence}
