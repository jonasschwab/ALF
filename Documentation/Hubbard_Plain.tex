...
logical               :: Adiabatic = .false. ! If  true  and  if  the  projector is true  
                                             ! the  Hubbard interaction  will be  ramped 
                                             ! from  0 to Ham_U  in the  time interval 
                                             ! 0  to  theta
...
We set the dimension for the interaction term to  $N=1$, and  allocate and initialize this array of type  \texttt{Operator} by repeatedly calling the subroutine \texttt{Op\_make}.     Here  we  also  allow  for the possibility  of  switching on  adiabatically  the  Hubbard  interaction 
provided  that     \texttt{Adiabatic=.true.}   and  that  we  are  working  with the   projector    method.  The   protocol   we  use  is   depicted in
Fig.~\ref{U_profile.fig}.   Note  that  to    trigger  the  time   dependence  \texttt{Op\_V\%g\_t}   has  to  be allocated,  and  the   index of  
the complex valued   one-dimensional  array    has  to run from   $1$  to  \texttt{Ltrot}. 

\begin{figure}
\center 
\includegraphics[width=0.6\textwidth]{U_profile.pdf}

        \caption{Protocol  for   wrapping  up  the  Hubbard  U-term  for   in  the  projective  algorithm. This approach  has  been  put  forward as  a
         method  to  alleviate  the negative sign problem   \cite{Vaezi21}.    }
        \label{U_profile.fig}
\end{figure}
...
\texttt{Op\_V(i,nf)}     encodes    $ e^{  X \sqrt{\Delta   \tau U/2} \left( \hat{c}^{\dagger}_{\ve{i},{\texttt{nf}}}  \hat{c}^{\phantom\dagger}_{\ve{i},{\texttt{nf}}}  + \alpha  \right) }  $    with  $X=1 $  for  $\texttt{nf}=1$  and $X=-1 $  for  $\texttt{nf}=2$.    Strictly  speaking  $X$  can  be  omitted.  However,  it  is  required  when  using the  flavor  symmetry  option in the presence  of  particle-hole  symmetry (see  Sec.~\ref{sec:flavor_sym}).     If  the  variable  
\texttt{Adiabatic=.true.}     and  \texttt{Projector=.true.}   then the   imaginary time    profile  of  the  Hubbard interaction  is  given by   Fig.~\ref{U_profile.fig}
