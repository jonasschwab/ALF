...
\end{lstlisting}
in which the operator array \path{OP_T(*,N_FL)} is allocated and defined. In the simplest case, where no checkerboard is used, the array's first dimension is unity. This routine allows optional 
arguments to allow  for pinning: that is, modifying existing  matrix elements in the hopping matrix. The syntax is the following: 
\begin{itemize}
\item \texttt{pinned\_vertices[1:N\_pin,1:2]}  contains the two legs $(i,\delta)$ and  $(j,\delta')$ of the bond or vertex to be pinned.  If $(i,\delta) = (j,\delta')$  then the pinning refers to a site and not to a bond.
\item \texttt{pinning\_factor[1:N\_pin,1:N\_fl]} contains the multiplicative factor for each flavor. 
\item \texttt{pinning\_offset[1:N\_pin,1:N\_fl]} contains the offset for each flavor. 
\end{itemize}
