...
and   takes  values  $ x \in \left[ 0,1 \right] $. Consider 
\begin{equation}
...
\begin{equation}
    ...
    \overline{g(\tau)} = M_0 \int_{0}^{1} \d{x} K(\tau,\omega^{-1}(x))  n(x).
\end{equation}
...
The  specification of the  default model is optional.   If the file  \texttt{Default}  exists  then this  default  will be   used.  We note that the default model  corresponds to the output of the MaxEnt code. That is $S(q,\omega)$ for particle-hole quantities,  $A(k,\omega)$ in the particle channel, etc. eThe  format 
...
\begin{equation}
 G(\omega) =  -\frac{1}{\pi} \int_{-\infty}^{\infty} \d \Omega   \frac{A(\Omega)}{\omega - \Omega + i \delta} \,,
\end{equation}
...
The first column of the  \texttt{Green}  file is a placeholder for post-processing. The last three columns   correspond to $\omega, \Re G(\omega) ,   \Im G(\omega) $. 
...
\langle \langle \big\{ \hat{c}^{\phantom\dagger}_{k},   \hat{c}^{\dagger}_{k}    \big\} \rangle \rangle_{C}   = 1$, hence, 
if both  the $\tau=0$ and $\tau=\beta$ data points are included, the covariance matrix will have a zero eigenvalue and the $\chi^{2}$ measure is not defined. 
Therefore, for the particle channel the program omits the $\tau=\beta$ data point. There are special  particle-hole symmetric  cases where the $\tau=0$ data 
point shows no  fluctuations -- in such cases the code omits the $\tau=0$ data point as well.
...
\end{equation}
Since the linear response to a hermitian perturbation  is real, $\chi''(q,\omega)  = - \chi''(-q,-\omega)$ and hence 
$\langle \hat{S}(q,\tau) \hat{S}(-q,0) \rangle $ is a symmetric function around $\beta= \tau/2$ for systems with 
inversion symmetry -- the ones we consider here.  When  \texttt{Channel=PH}  the analysis program \texttt{ana.out} uses this symmetry to provide an improved estimator. 
...
\begin{equation}
  \label{eqn:A_ph_def}
	\langle \hat{S}(q,\tau) \hat{S}(-q,0) \rangle  =  
       ...
       \underbrace{ \coth(\beta \omega/2) \chi''(q,\omega) \vphantom{\frac{1}{2-1}} }_{A_{ph}(\omega)} 
\end{equation}
...
\begin{equation}
	\langle \hat{S}(q,\tau) \hat{S}(-q,0) \rangle  =  
       \int_{0}^{\infty}  \d \omega \underbrace{\left( K_{pp}(\tau,\omega)  + K_{pp}(\tau,-\omega) \right)}_{K_{ph}(\tau,\omega)}  A_{ph}(\omega).
\end{equation}
...
\begin{equation}
	S(q,\omega) = \frac{A_{\text{ph}}(\omega)}{1 + e^{-\beta \omega} } .
\end{equation}
...
\subsection{Optical conductivity: \texttt{Channel=PH\_C}}

A  slight modification of the above is required to compute  the optical conductivity. In the Lehmann representation, this quantity reads:
\begin{equation}
  \sigma'(\omega) =  \frac{\pi}{Z} \frac{1}{\omega} \sum_{n,m} e^{-\beta E_n} |\langle n | \hat{J}_p | m \rangle |^2 
  \delta ( \omega + E_n - E_m) \left( 1 - e^{-\beta  \omega} \right) 
\end{equation}
where  $\hat{J}_p$ is a model dependent paramagnetic current operator.  One can show that the sum rule, 
\begin{equation}
	\int \d \omega 	 \omega \coth(\beta \omega/2) \sigma'(\omega) = 
  2 \pi \langle \hat{J}_p(\tau=0) \hat{J}_p(0) \rangle
\end{equation}
holds  such  that: 
\begin{equation}
  \label{eqn:A_ph_C_def}
	\langle \hat{J}(\tau) \hat{J}(0) \rangle  =  
       \int \d \omega  \underbrace{ \frac{1}{\pi} \frac{e^{- \tau \omega} }
            { 1 - e^{-\beta  \omega} } \tanh(\beta \omega/2)  }_{K_{pp}(\tau,\omega)} 
       \underbrace{ \omega \coth(\beta \omega/2) \sigma'(\omega) \vphantom{\frac{1}{2-1}} }_{A_C(\omega)} 
\end{equation}
and one computes $A_C(\omega)$. Since  $\hat{J}_p $  is hermitian, $\sigma'(\omega)$ is an even function of frequency, the above can be simplified to 
\begin{equation} 
    \langle \hat{J}_p(\tau) \hat{J}_p(0) \rangle  =  
         \int_{0}^{\infty}  \d \omega \underbrace{\left( K_{pp}(\tau,\omega)  + K_{pp}(\tau,-\omega) \right)}_{K_{ph}(\tau,\omega)}  A_C(\omega).
\end{equation}
As apparent,  the only difference with the particle-hole channel of Sec.~\ref{sec:maxent_ph} is in the 
back transformation routine \texttt{BACK\_TRANS\_ph\_C}  required to obtain the optical conductivity: 
\begin{equation}
	\sigma'(\omega) =  \frac{\tanh\left( \beta \omega/2\right)}{\omega}A_C(\omega)
\end{equation} 

\subsection{Particle-Particle quantities: \texttt{Channel=PP}}

Similarly to the particle-hole channel, the particle-particle channel is also a bosonic correlation function. Here, however, we do not assume that the  imaginary time data is symmetric around   the $\tau = \beta/2$ point.  We use the kernel $K_{pp}$ 
defined in Eq.~\eqref{eqn:A_ph_def}  and consider the whole frequency range. 
The back transformation  yields
\begin{equation}
 \frac{\chi''(\omega)} {\omega}   = \frac{\tanh \left( \beta \omega/2 \right) }{ \omega }   A(\omega) .
\end{equation}
If no default is provided, then we use a flat default for the $A(\omega)$ that satisfies the sum rule: $ \int d \omega A(\omega) =  \pi  \left(\langle \hat{S}(q,\beta) \hat{S}(-q,0) \rangle  +
\langle \hat{S}(q,0) \hat{S}(-q,0) \rangle \right) $. Here, we use the same notation as in Eq.~\eqref{eqn:A_ph_def}.
...
\subsection{ Integration  over  the  spectral  function   } 

In many cases,  one  would like  to  compute  the   integral
\begin{equation}
(F,A) = \int d \omega F(\omega) A(\omega)
\end{equation} 
for  a  user  defined  function $F(\omega) $   and  the  spectral  function $A(\omega)$ produced  in the   stochastic  MaxEnt.  An example of an application
where  this  can  come in handy  is  the  calculation of  the    quantum  Fisher  information (QFI) \cite{Hauke16}  or susceptibilities. 
As   seen   above,  the   stochastic MaxEnt   encodes  the  image $n(x)$ as  a  set  of   Dirac $\delta$-functions  (Eq.~\eqref{eqn:n_parametrization}); thus  
rendering the  integration   very  convenient to  carry  out.  Since we  stochastically  sample  the image, we  can easily estimate  
the  fluctuations of  the result.    In fact,  
\begin{equation}
  (F,A)     =  M_0 \int_{0}^{1}d x  F(\Phi^{-1}(x)) n(x) . 
\end{equation}
The  function  $ F(\omega) $  is  provided  by  the  user  in the  call to  the stochastic  MaxEnt  routine.  The  program  then 
computes $(F,A) $ at  the  given  sets  of   temperatures  $\alpha$ (see Eq.~\ref{eqn:Stoch_MaxEnt})   provided  by  the user.    

One  can use  the above  to   efficiently  compute susceptibilities---   typically  of the form: 
\begin{equation}
   \chi  =   \int_{0}^{\beta } d \tau   g(\tau)      =   \int d  \omega \underbrace{ \int_{0}^{\beta}  d  \tau  K(\tau, \omega)  }_{\equiv F(\omega)}   \, A (\omega). 
\end{equation}
Here,  the   analytic  continuation is  used  as   an  optimal fit  function  to  the  imaginary time  data,  allowing the  minimization of   finite  
discretization  time  errors. 

Another  example is  the  quantum Fisher  information \cite{Hauke16}.  In the particle-hole channel,  this  quantity  reads: 
\begin{equation}
   F_Q(T) =  \int_{0}^{\infty}   d \omega   \frac{4}{\pi} \tanh\left( \frac{\beta \omega}{2} \right) \chi^{''}(\omega)  = 
             \int_{0}^{\infty}   d \omega  \underbrace{ \frac{4}{\pi} \tanh^2\left( \frac{\beta \omega}{2} \right) }_{\equiv F(\omega) } \, A(\omega).
\end{equation}
The  last  step follows  for  Eq.~\ref{eqn:A_ph_def}.    Results  are   written  in the file  \texttt{(F,A).dat}.  The file  has 
three  rows corresponding to inverse  temperature $\alpha$, $(F,A)$   and  $\Delta (F,A)$.  The  function  $F$  is  specified  in the 
MaxEnt wrapper  \texttt{Max\_SAC.F90}  in the   \texttt{Analysis}   directory.

 \subsection{Dynamics of the one-dimensional half-filled Hubbard model}

To conclude this section, we show the example of the one-dimensional Hubbard model, which is known to show spin-charge separation (see Ref.~\cite{Abendschein06}  
and references therein).    The data of Fig.~\ref{Fig:Spectral1D}    was produced with the pyALF python script  
\href{https://git.physik.uni-wuerzburg.de/ALF/ALF/-/blob/\ALFbranch/Documentation/Figures/MaxEnt/Hubbard_1D.py}{\texttt{Hubbard\_1D.py}}, and   the spectral function plots with the bash script \href{https://git.physik.uni-wuerzburg.de/ALF/ALF/-/blob/\ALFbranch/Documentation/Figures/MaxEnt/Spectral.sh}{\texttt{Spectral.sh}}. 
