...
\end{equation}
If no default is provided, then we will use a flat default for the $A(\omega)$ that satisfies the sum rule: $ \int d \omega A(\omega) =  \pi  \left(\langle \hat{S}(q,\beta) \hat{S}(-q,0) \rangle  +
\langle \hat{S}(q,0) \hat{S}(-q,0) \rangle \right) $. Here, we have used the same notation as in 
Eq.~\eqref{eqn:A_ph_def}.
