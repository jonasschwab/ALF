...
HMC                  = .F.  ! Flag for hybrid molecular dynamics  update
N_HMC_sweeps         = 1    ! Number of hybrid molecular dynamic  updates between 
                            ! sequential sweep
Leapfrog_steps       = 0    ! Number of leapfrog iterations 
Amplitude            = 1.0  ! Width of the box distribution for update of 
...
Extended_zone =  .F.        ! Allows for Fourier analysis in extended zone scheme 
N_BZ_Zones    =   1         ! Number of Brillouin zones that will be covered if extended 
                            ! zone scheme is activated. Default = Norb
/  
...
&VAR_Max_Stoch             !! Variables for  Maximum entropy
                            ! The  attribute Stoch means  that this  parameter is 
                            ! only  required for  the  stochastic  approach. 
Ngamma     = 400            ! Number of delta-functions for parametrization (Stoch)
Om_st      = -10.d0         ! Frequency range lower bound
Om_en      = 10.d0          ! Frequency range upper bound
NDis       = 2000           ! Number of boxes for histogram, frequency  discretization.  
Nbins      = 250            ! Number of bins for Monte Carlo (Stoch)
Nsweeps    = 70             ! Number of sweeps per bin  (Stoch)
NWarm      = 20             ! The Nwarm first bins will be omitted (Stoch)
N_alpha    = 14             ! Number of temperatures (Stoch)
alpha_st   = 1.d0           ! Smallest inverse temperature increment for inverse (Stoch)
R          = 1.2d0          ! temperature (see above) (Stoch) 
Checkpoint = .F.            ! Whether to produce dump files, allowing the simulation
                            ! to be resumed later on (Stoch)
Tolerance  = 0.1d0          ! Data points for which the relative error exceeds the
                            ! tolerance threshold will be omitted.
Stochastic =.T.             ! If  true,  stochastic  maxent  is  called
                            ! If  false, classic  maxent  is  called
/
...
\begin{table}[h]
   ...
   \path{Y_eq, Y_tau, Y_local} & Results of equal-time and time-displaced measurements of correlation functions. The placeholder \path{Y} stands for \path{Green}, \path{SpinZ}, \path{SpinXY}, \path{Den}, etc. \\   
   \path{Y_eq_info, Y_tau_info, Y_local} & Additional info, like Bravais lattice and unit cell, for equal-time and time-displaced observables \\
...
\dirtree{%
.1 data.h5.
.2 lattice\DTcomment{Attached attributes describe Bravais lattice and unit cell}.
.2 X\_scal\DTcomment{Attached attribute: analysis\_mode}.
.3 obser\DTcomment{Dataset of shape (NBins, Nobs, 2)}.
.3 sign\DTcomment{Dataset of shape (NBins)}.
.2 Y\_eq Y\_tau\DTcomment{Attached attributes: Channel, dtau}.
.3 lattice\DTcomment{Attached attributes describe Bravais lattice and unit cell}.
.3 obser\DTcomment{Dataset of shape (NBins, Norbs, Norbs, Ntau, Nlatt, 2)}.
.3 back\DTcomment{Dataset of shape (NBins, Norbs, 2)}.
.3 sign\DTcomment{Dataset of shape (NBins)}.
.2 parameters.
.3 namelist\_1\DTcomment{Attached attributes are the parameters in namelist\_$1$}.
.3 \hspace{20pt}\vdots.
.3 namelist\_n\DTcomment{Attached attributes are the parameters in namelist\_$n$}.
}
