...
Alternatively, one could use \href{https://hub.docker.com/r/alfcollaboration/jupyter-pyalf-full}{this Docker image}, which has ALF, pyALF and a Jupyter server pre-installed.
...
git clone -b #\ALFbranch#https://git.physik.uni-wuerzburg.de/ALF/ALF.git
cd ALF
...
\begin{lstlisting}[style=bash]
          1 -0.29821914139694449E+002  0.13032001866398857E+000
\end{lstlisting}
...
\begin{table}[!ht]
	\begin{center}
		\begin{tabular}{@{} p{0.20\columnwidth} p{0.78\columnwidth} @{}}\toprule %16 81
			...
			\texttt{PARALLEL\_PARAMS}  & Run multiple independent simulations in parallel (Sec.~\ref{sec:parallel_params})
			\vspace{7pt}\\ 
			...
			\textbf{Environment Variables} & These are not arguments, but rather environment variables that will be recognized by \texttt{configure.sh}  \\\midrule
			\texttt{ALF\_FLAGS\_EXT}  & Hand these additional flags to the compiler\\
			\texttt{ALF\_HDF5\_DIR}  & ALF usually self-compiles HDF5 and stores the library in subdirectories of \texttt{ALF/HDF5},
			                         this behavior can be changed by setting the environment variable \texttt{ALF\_HDF5\_DIR}\\
			\bottomrule
		\end{tabular}
...
\begin{table}[h]
	\begin{center}
		...
		\texttt{Y\_eq\_eJK} & Jackknife mean and error of \texttt{Y}, which stands for \texttt{Green, SpinZ, SpinXY}, or \texttt{Den}. The file contains the analysis in the extended zone scheme 
		(See Eq.~\ref{eq:extended_zone})\\	
		...
		\texttt{Y\_e\_kx\_ky/g\_kx\_ky} & Time resolved and $\vec{k}$-dependent Jackknife mean and error of \texttt{Y}, where \texttt{Y} stands for \texttt{Green, SpinZ, SpinXY}, and \texttt{Den}.
		The file contains the analysis in the extended zone scheme 
		(See Eq.~\ref{eq:extended_zone})\\
...
\begin{itemize}
	...
	\item For the equal-time correlation functions  in   real  space,  \texttt{Y}, the formatting of the output files \texttt{Y\_eqJR}  follows the structure:
	{\small \begin{alltt}
		do i = 1, N_unit_cell
		   <r_x(i)>   <r_y(i)>
	...
	\item  The  format  for   equal  time  correlations in  Fourier  space  depends  upon    the   boolean  variable   \texttt{Extended\_Zone}.  
	 For  \texttt{Extended\_Zone=.F.},   the  format  reads: 
	 {\small \begin{alltt}
		do i = 1, N_unit_cell
		   <k_x(i)>   <k_y(i)>
		   do alpha = 1, N_orbital
		...
		enddo
	\end{alltt} }
	where \texttt{Re} and \texttt{Im} refer to the real and imaginary part, respectively.  The  Fourier   transformation  is   
	defined  in Eq.~\ref{eq:fourier_transform}.  

	For  \texttt{Extended\_Zone=.T.},   the  format  reads: 
	 {\small \begin{alltt}
		do n = 0, N_BZ_Zones -1 
		   do m = 0, N_BZ_Zones 	-1
		        do  i  =  1,N_unit_cell 
		              <k_x(i) +  n b_1 + m b_2 >   <k_y(i) + n b_1 +  m b_2 >   & 
		                     &    Re<mean(Y)>  Re<error(Y)>  Im<mean(Y)>  Im<error(Y)>
		        enddo
		   enddo
		enddo
	\end{alltt} }
	In  this  case,  the  Fourier  transformation is    defined  as: 
	\begin{equation}
		S( \ve{k} +  n \ve{b}_1  + m \ve{b}_2)   =    \sum_{\alpha,\beta} e^{- i \left( \ve{k} +  n \ve{b}_1  + m \ve{b}_2 \right) \cdot   \left( \ve{r}_{\alpha} -  \ve{r}_{\beta} \right)  } S_{\alpha,\beta}(\ve{k} ).  
		\label{eq:extended_zone}
	\end{equation}   
	The  positions  of  the orbitals, $\ve{r}_{\alpha}$  within the  unit  cell  are   read in from  the  file  \text{Y\_info},  $\ve{b}_1$, $\ve{b}_2$, are  the  
	reciprocal lattice  vectors  and  the Fourier  transform is  defined in equation  Eq.~\ref{eq:fourier_transform}.
	The naming of the output files equally depends upon the boolean variable \texttt{Extended\_Zone} (see table \ref{table:analysis_files}).
	...
	$0$ to $\beta/2$.   In all other cases it runs from $0$ to $\beta$.     The  above  is  valid   for  \texttt{Extended\_Zone=.F.}.   If \texttt{Extended\_Zone=.T.} then   \texttt{Tr[Y] }    is  replaced by the  Fourier   transform in the  extended  zone  scheme  as   defined  above,   but  for time-displaced  correlation   functions. 
...
%
%-------------------------------------------------------------------------------------
\subsection{Parallel parameters feature} \label{sec:parallel_params}
%-------------------------------------------------------------------------------------
%
With the compile-time option \verb|PARALLEL\_PARAMS| (or equivalently \verb|PP|), ALF can run multiple independent simulations in parallel. This feature is meant for utilizing the high core counts of modern HPC systems in cases where a normal job size might be overkill for a single simulation. The basic folder structure is the same as in Parallel Tempering, as sketched in Fig.~\ref{fig:parallel_params}: The root directory contains the files \verb|parameters|, \verb|seeds| and directories \verb|Temp_0| to \verb|Temp_{N-1}|, where $N$ is the number of parameter sets, i.e. different simulations. Contrary to Parallel Tempering, in Parallel Parameters mode only \verb|mpi_per_parameter_set| from the namelist \verb|VAR_TEMP| is read from \verb|./parameters|. This variable is defined as the number of availbale MPI processes divided by $N$. All the rest of the parameters are read from \verb|Temp_*/parameters|, which allows to simulate completely different parameters and even different Hamiltonians.
...
\caption{Folder structure for a parallel parameters (or Parallel Tempering) simulation with 2 parameter sets.}\label{fig:parallel_params}
\end{figure}
