...
\begin{itemize}
	...
	$0$ to $\beta/2$.   In all other cases it runs from $0$ to $\beta$.     The  above  is  valid   for  \texttt{Extended\_Zone=.F.}.   If \texttt{Extended\_Zone=.T.} then   \texttt{Tr[Y] }    is  replaced the  Fourier   transform in the  extended  zone  scheme  as   defined  above   but  for 
