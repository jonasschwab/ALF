...
%-------------------------------------------------------------------------------------
%
With the compile-time option \verb|PARALLEL\_PARAMS|, ALF can run multiple independent simulations in parallel. This feature is meant for utilizing the high core counts of modern HPC systems in cases where a normal job size might be overkill for a single simulation. The basic folder structure is the same as in Parallel Tempering, as sketched in Fig.~\ref{fig:parallel_params}: The root directory contains the files \verb|parameters|, \verb|seeds| and directories \verb|Temp_0| to \verb|Temp_{N-1}|, where $N$ is the number of parameter sets. Contrary to Parallel Tempering, in Parallel Parameters mode only \verb|mpi_per_parameter_set| from the namelist \verb|VAR_TEMP| is read from \verb|parameters|. All the rest of the parameters are read from \verb|Temp_*/parameters|, which allows to simulate completely different parameters and even different Hamiltonians.
