...
\subsection{SU(N) Hubbard models \texttt{Hamiltonian\_Hubbard\_smod.F90}} \label{sec:hubbard}
...
ham_h0    = 0.d0            ! Pinning. Local magnetic field on site i=0, orbital=1
/
...
\hat{H} =& \sum_{(\ve{i},\ve{\delta}), (\ve{j},\ve{\delta}')}  \sum_{\sigma =1}^{N}  T_{(\ve{i},\ve{\delta}), (\ve{j},\ve{\delta}')}    \hat{c}^{\dagger}_{(\ve{i},\ve{\delta}), \sigma }   e^{\frac{2 \pi i}{\Phi_0} \int_{\ve{i} + \ve{\delta}}^{\ve{j} + \ve{\delta}'}  
  ...
  & + \sum_{\vec{i}} \sum_{\delta}   \frac{U_{\ve{\delta}} }{N} \left(\sum_{\sigma=1}^{N}  \left[   \hat{c}^{\dagger}_{(\vec{i},\ve{\delta}),\sigma } 
    ...
    &  + h_0 \left(\sum_{\sigma=1}^{N/2} \hat{c}^{\dagger}_{(\vec{0},\ve{1}),\sigma, 1}  \hat{c}^{}_{(\vec{0},\ve{1}),\sigma, 1}  -  \sum_{\sigma=N/2}^{N} \hat{c}^{\dagger}_{(\vec{0},\ve{1}),\sigma, 2}  \hat{c}^{}_{(\vec{0},\ve{1}),\sigma, 2} \right) .
\end{align}
The generic hopping is taken from Eq.~\eqref{generic_hopping.eq}   with appropriate boundary conditions given by Eq.~\eqref{generic_boundary.eq}.  The index $\ve{i}$ runs over the unit cells, $\ve{\delta}$ over the orbitals in each unit cell and $\sigma$  from $1$ to $N$  and encodes the SU(N) symmetry.    Note that  $N$ corresponds to \texttt{N\_SUN}  in the code.  The chemical potential $\mu$ is relevant only for the finite temperature code. Generically the flavor index is set to  unity.  However, if the pinning field, $h_0$, is  included, then the symmetry of the Hamiltonian is reduced from U(N) to U(N/2) $\times$ U(N/2). In this case, one 
should set $N\_{\mathrm{fl}}=2$. 
The term \texttt{ham\_h0}  accounts for a  local magnetic field at site $\ve{i}=0$ and orbital $\ve{\delta}=1$.
This is useful to probe for e.g. long-ranged magnetic order as discussed in Ref.~\cite{Assaad13}.  Note that $N$ is required to be even if the pinning field is included. 


If the variable \texttt{Mz} is set to \texttt{.True.}, then the code equally requires  \texttt{N\_SUN}  to be even and simulates the following Hamiltonian: 
\begin{align}
\hat{H} =& \sum_{(\ve{i},\ve{\delta}), (\ve{j},\ve{\delta}')}  \sum_{\sigma =1}^{N/2}  \sum_{s=1,2} T_{(\ve{i},\ve{\delta}), (\ve{j},\ve{\delta}')}    \hat{c}^{\dagger}_{(\ve{i},\ve{\delta}), \sigma,s }   e^{\frac{2 \pi i}{\Phi_0} \int_{\ve{i} + \ve{\delta}}^{\ve{j} + \ve{\delta}'}  
    ...
    &  + h_0 \left(\sum_{\sigma=1}^{N/2} \hat{c}^{\dagger}_{(\vec{0},\ve{1}),\sigma, 1}  \hat{c}^{}_{(\vec{0},\ve{1}),\sigma, 1}  -  \sum_{\sigma=N/2}^{N} \hat{c}^{\dagger}_{(\vec{0},\ve{1}),\sigma, 2}  \hat{c}^{}_{(\vec{0},\ve{1}),\sigma, 2} \right) .
\end{align}
Note that if \texttt{Mz=.T.} then the code automatically sets  \texttt{N\_fl=2}. Cleary at $N=2$, both modes  correspond  to the Hubbard model.  For $N$  even and $N > 2$  the models differ.  In particular  in the latter  Hamiltonian the U(N) symmetry is broken down to  U(N/2) $\times$ U(N/2).  When $h_0 \neq 0$  a local pinning field is applied at site $\ve{i}=0$ and orbital $\ve{\delta}=1$  to break the U(N) symmetry explicitly.   This is useful to probe for e.g. long-ranged magnetic order as discussed in Ref.~\cite{Assaad13}.
...
\subsection{SU(N)  t-V models  \texttt{Hamiltonian\_tV\_smod.F90}}
...
\href{https://git.physik.uni-wuerzburg.de/ALF/pyALF/-/blob/\pyALFbranch/Notebooks/tV_model.ipynb}{\texttt{tV\_model.ipynb}}.
