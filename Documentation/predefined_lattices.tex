% !TEX root = doc.tex
% Copyright (c) 2017-2021 The ALF project.
...
In order to carry out simulations on the Honeycomb lattice, which is a triangular Bravais lattice with two orbitals per unit cell, choose \path{Lattice_type = "Honeycomb"}\index{\path{Honeycomb}}, which sets
\begin{lstlisting}[style=fortran]
...
\subsubsection{Triangular lattice, Fig.~\ref{fig_predefined_lattices}(f)}
The \texttt{"Triangular"}\index{\texttt{Triangular}} configuration sets:
\begin{lstlisting}[style=fortran]
Latt_Unit%Norb    = 1
Latt_Unit%N_coord = 3
a1_p(1) =  1.D0   ; a1_p(2) =  0.d0
...
Call Make_Lattice( L1_p, L2_p, a1_p,  a2_p, Latt )
\end{lstlisting}
This lattice is only implemented  for the Hubbard model. 

\subsubsection{Kagome lattice, Fig.~\ref{fig_predefined_lattices}(g)}
The \texttt{"Kagome"}\index{\texttt{Kagome}} configuration sets:
\begin{lstlisting}[style=fortran]
Latt_Unit%Norb    = 3
Latt_Unit%N_coord = 4 
a1_p(1) =  1.D0   ; a1_p(2) =  0.d0
